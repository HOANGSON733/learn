"use client"

import { useEffect, useRef, useState } from "react"
import {
  AnimatePresence,
  motion,
  useMotionValue,
  type HTMLMotionProps,
} from "motion/react"

import { cn } from "@/src/lib/utils"

/**
 * A custom pointer component that displays an animated cursor.
 * Add this as a child to any component to enable a custom pointer when hovering.
 * You can pass custom children to render as the pointer.
 *
 * @component
 * @param {HTMLMotionProps<"div">} props - The component props
 */
export function Pointer({
  className,
  style,
  children,
  ...props
}: HTMLMotionProps<"div">): React.ReactNode {
  const x = useMotionValue(0)
  const y = useMotionValue(0)
  const [isActive, setIsActive] = useState(false)
  const containerRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    const parentElement = containerRef.current?.parentElement

    if (!parentElement) return

    const pointerStyleId = "pointer-cursor-style"
    const existingStyle = document.getElementById(pointerStyleId)
    const styleTag = existingStyle ?? document.createElement("style")

    if (!existingStyle) {
      styleTag.id = pointerStyleId
      styleTag.textContent = `
        html.pointer-active,
        html.pointer-active * {
          cursor: none !important;
        }
      `
      document.head.appendChild(styleTag)
    }

    const setPointerActive = () => {
      x.set(window.event instanceof PointerEvent ? window.event.clientX : x.get())
      y.set(window.event instanceof PointerEvent ? window.event.clientY : y.get())
      setIsActive(true)
      document.documentElement.classList.add("pointer-active")
    }

    const handlePointerMove = (e: PointerEvent) => {
      x.set(e.clientX)
      y.set(e.clientY)
      setPointerActive()
    }

    const handlePointerEnter = (e: PointerEvent) => {
      x.set(e.clientX)
      y.set(e.clientY)
      setPointerActive()
    }

    const handlePointerLeave = () => {
      setIsActive(false)
      document.documentElement.classList.remove("pointer-active")
    }

    parentElement.addEventListener("pointermove", handlePointerMove)
    parentElement.addEventListener("pointerenter", handlePointerEnter)
    parentElement.addEventListener("pointerleave", handlePointerLeave)

    return () => {
      document.documentElement.classList.remove("pointer-active")
      parentElement.removeEventListener("pointermove", handlePointerMove)
      parentElement.removeEventListener("pointerenter", handlePointerEnter)
      parentElement.removeEventListener("pointerleave", handlePointerLeave)

      if (!existingStyle) {
        styleTag.remove()
      }
    }
  }, [x, y])

  return (
    <>
      <div ref={containerRef} />
      <AnimatePresence>
        {isActive && (
          <motion.div
            className="pointer-events-none fixed z-50 transform-[translate(-50%,-50%)]"
            style={{
              top: y,
              left: x,
              ...style,
            }}
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0, opacity: 0 }}
            {...props}
          >
            {children || (
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth="1"
                viewBox="0 0 16 16"
                height="24"
                width="24"
                xmlns="http://www.w3.org/2000/svg"
                className={cn(
                  "rotate-[-70deg] text-foreground drop-shadow-sm",
                  className
                )}
              >
                <path d="M14.082 2.182a.5.5 0 0 1 .103.557L8.528 15.467a.5.5 0 0 1-.917-.007L5.57 10.694.803 8.652a.5.5 0 0 1-.006-.916l12.728-5.657a.5.5 0 0 1 .556.103z" />
              </svg>
            )}
          </motion.div>
        )}
      </AnimatePresence>
    </>
  )
}
